...
public class TweetController {
	...
	public ResponseEntity<Response> updateTweet(@PathVariable("username") String username, @PathVariable("id") String id, @RequestBody Tweets tweet) {
		Response response = tweetService.updateTweet(username, id, tweet);
		return new ResponseEntity<Response>(response, HttpStatus.OK);
	}
	
	@DeleteMapping("/{username}/delete/{id}")
	public ResponseEntity<Response> deleteTweet(@PathVariable("username") String username, @PathVariable("id") String id) {
		Response response = tweetService.deleteTweet(username, id);
		return new ResponseEntity<Response>(response, HttpStatus.OK);
	}
	
	@PutMapping("/{username}/like/{id}")
	public ResponseEntity<Response> likeTweet(@PathVariable("username") String username, @PathVariable("id") String id) {
		Response response = tweetService.likeTweet(username, id);
		return new ResponseEntity<Response>(response, HttpStatus.OK);
	}
	
	@PostMapping("/{username}/reply/{id}")
	public ResponseEntity<Response> replyToTweet(@PathVariable("username") String username, @PathVariable("id") String id, @RequestBody Reply reply) {
